...
- N: 模型的状态数量。对于实际问题，可能无法确定N。定义各个状态为 $ S=\{S_1, S_2,...,S_N\} $，$t$ 时刻的状态定义为 $\bold{q_t}$ .
...
2. 平滑(smoothing)：已知模型参数和某一特定输出序列，求中间时刻各个隐含**状态**的概率分布，即求$  P(x(k) |  y(1),\dots  , y(t) ), \ k<t$. 通常使用前向-后向算法解决.
