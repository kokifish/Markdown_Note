...
![](http://op4fcrj8y.bkt.clouddn.com/18-4-25/94757368.jpg)
...
| Flags(DF,MF)   | 3    | DF: Don’t Fragment(1:不分片) MF: More Fragment(1:还有分片)   |
...
![](http://op4fcrj8y.bkt.clouddn.com/18-7-13/32782011.jpg)
...
- 头部长度 Data offset : 以32bit(4B)为单位，头部长度的实际大小为[20, 60] Bytes,故选项字段最大40B
...
![](http://op4fcrj8y.bkt.clouddn.com/18-7-9/92886685.jpg)

伪IP头：前96 Bytes为**伪IP头**，加入校验和中做加法用。UDP运行在IPv4之上时，为了能够计算校验和，需要在UDP数据包前添加一个“伪头部”。伪头部包括了IPv4头部中的一些信息，但它并不是发送IP数据包时使用的IP数据包的头部，而只是一个用来计算校验和而已 

![](http://op4fcrj8y.bkt.clouddn.com/18-7-9/45520611.jpg)

> 粉色部分为伪UDP头，用于计算Check Sum，传输时去掉。
